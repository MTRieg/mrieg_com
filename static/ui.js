...
async function fetchGameState(ReplayLastTurn=false) {
  ...
  console.log("ReplayLastTurn:", ReplayLastTurn, " old_pieces:", Boolean(data.old_pieces && data.old_pieces.length > 0));

  // store current turn for submissions
  if (typeof data.turn_number === "number") currentTurn = data.turn_number;
  if (typeof data.board_size === "number") board_size = data.board_size;
  if (typeof data.board_shrink === "number") board_shrink = data.board_shrink;

  if (data.old_pieces && data.old_pieces.length > 0 && ReplayLastTurn) {
    board_size += board_shrink;
    await place_pieces(data.old_pieces, data.players);
    console.log(`Loaded ${data.old_pieces.length} old pieces from server, will start physics shortly`);
  ...
  if (data.pieces && data.pieces.length > 0) {
    ...
    console.log(`Loaded ${data.pieces.length} pieces from server`);
    return;
  } 
  defaultLoad();
