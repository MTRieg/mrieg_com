...
import { canvas, pendingArrows, players, physics, gameId, playerId, submitMoves, fetchGameState, setPlayerId, getCurrentTurn} from "./ui.js";
...
export function createButtonPanel(container) {
  ...
  btnRunTurn.addEventListener("click", async () => {
    try {
      ...
      // Get current turn number from client-side game state
      const currentTurnNum = getCurrentTurn();
      
      const response = await fetch(`${API_BASE}/apply_moves_and_run_game`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ game_id: gameId, player_id: playerId, turn_number: currentTurnNum })
      });
