...
export async function initialize() {
  ...
  minimal.updateGameInfo(existingGameId, `mrieg.com/games/knockout?game_id=${existingGameId}`, gameData.next_turn_deadline);
...
async function viewExistingGame(gameId, password) {
  ...
  // Change status to player setup plus show players
  minimal.statusControl.status = "pi";
  minimal.updateGameInfo(gameId, `mrieg.com/games/knockout?game_id=${gameId}`, data.next_turn_deadline);
...
async function registerWithExistingGame(gameId, player_id, password) {
  ...
  if (res.ok) {
    ...
    minimal.updateGameInfo(gameId, `mrieg.com/games/knockout?game_id=${gameId}`, res.start_time)
    startPolling(gameId)
...
async function loginWithExistingGame(gameId, username, password) {
  ...
  if (res.ok || res.status === 409) {
    ...
    //change status to show players
    minimal.statusControl.status = "i";
    minimal.updateGameInfo(gameId, `mrieg.com/games/knockout?game_id=${gameId}`, data.start_time)
    startPolling(gameId)
  } else {
    alert("Failed to join game.");
  }
