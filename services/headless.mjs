...
const sanitizeInput = (data) => {
  ...
  return {
    pieces: pieces.map((p, i) => {
      ...
      const radius = Number(p.radius ?? DEFAULT_RADIUS);
    ...
    boardBefore: Number.isFinite(data.boardBefore) ? data.boardBefore : DEFAULT_BOARD_SIZE,
    boardAfter: Number.isFinite(data.boardAfter) ? data.boardAfter : DEFAULT_BOARD_SIZE - DEFAULT_BOARD_SHRINK,
  };
};

async function main() {
  const data = await readStdin(200_000);
  let { pieces, boardBefore, boardAfter } = sanitizeInput(data);
